...
select RegistrationId, EntryType, TransactionType, ItemGroup, Type, ItemTypeId, Amount
from LedgerDetail l
join registrations r on l.RegistrationId = r.registrationId
where r.registrationId in (select registrationId from attenders where attended = 1)


Select l.* from LedgerDetail l
